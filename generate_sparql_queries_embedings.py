...
def find_best_query(user_question):
    ...
    user_embedding = np.array(get_embedding(user_question), dtype=np.float32).reshape(1, -1)
    distances, indices = faiss_index.search(user_embedding, k=1)
    best_query_index = indices[0][0]
    return queries_json["queries"][best_query_index]["query"], queries_json["queries"][best_query_index]["answer_value"]
